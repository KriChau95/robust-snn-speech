...
save_dirs = [
    ...
    "pitched_processed_spike_data"
]

label_map = {"cat": 0, "dog": 1}

# Define ann layers + forward
class ANN(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):
        # x: (B, 2, num_inputs, num_steps)
        x_rate = x.mean(dim=-1)               # (B, 2, num_inputs)
        x_rate = x_rate.view(x.size(0), -1)   # (B, 2*num_inputs)
...
def load_spike_data(save_dir, filename):
...
if __name__ == "__main__":
    ...
    for save_dir in save_dirs:
        ...
        X_train_pos = load_spike_data(save_dir, "X_train_pos.npy")
        X_train_neg = load_spike_data(save_dir, "X_train_neg.npy")
        y_train = np.load(os.path.join(save_dir, "y_train.npy"))

        X_test_pos = load_spike_data(save_dir, "X_test_pos.npy")
        X_test_neg = load_spike_data(save_dir, "X_test_neg.npy")
        y_test = np.load(os.path.join(save_dir, "y_test.npy"))

        print(f"Loaded from {save_dir}: "
              f"Train={len(X_train_pos)}, Test={len(X_test_pos)}")
    ...
    torch.save(net.state_dict(), "ann_pitched.pth")
