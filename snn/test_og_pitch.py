...
# Load trained SNN weights and set to evaluation mode
net = Net() 
net.load_state_dict(torch.load("snn_nondistorted.pth"))
net.eval()
...
# Helper to load spike data arrays from disk
...
# Build spike tensors + labels for each pitched training sample
...
# Build spike tensors + labels for each pitched test sample
...
with torch.no_grad():
    ...
    # Loop over all pitched samples and compute accuracy
    ...
    print("Accuracy on pitched data: {:.2f}%".format(test_acc * 100))
